...
SCALER_FILE  = os.path.join(OUT_DIR, "scaler_knn.pkl")
MODEL_FILE   = os.path.join(OUT_DIR, "knn_model.pkl")
SUMMARY_CSV  = os.path.join(OUT_DIR, "knn_summary.csv")
RANDOM_STATE = 42
...
# normalize pathology labels: lowercase
# CSV has uppercase labels like BENIGN_WITHOUT_CALLBACK, BENIGN, MALIGNANT
...
# encode target; map benign and malignant; map benign and malignant
label_map = {'benign': 0, 'malignant': 1}
df['target'] = df['pathology'].map(label_map)
# handle unknown labels by skipping them
if df['target'].isnull().any():
    unknown = df.loc[df['target'].isnull(), 'pathology'].unique()
    print(f"⚠️ Unknown pathology labels found and will be skipped: {unknown}")
