...
SCALER_FILE  = os.path.join(OUT_DIR, "scaler_hog_lbp_svm.pkl")
MODEL_FILE   = os.path.join(OUT_DIR, "svm_hog_lbp_model.pkl")
SUMMARY_CSV  = os.path.join(OUT_DIR, "svm_hog_lbp_summary.csv")
RANDOM_STATE = 42
...
feature_cols = [c for c in df.columns if c.startswith('lbp_') or c.startswith('hog_')]
...
print(f"✔️ Saved SVM model to {MODEL_FILE}")
