...
CSV_PATH     = "/data/final_cropped_full.csv"
BATCH_SIZE   = 32
LR_HEAD      = 1e-4
LR_FINETUNE  = 1e-5
EPOCHS_HEAD  = 10
...
for fold, (train_idx, val_idx) in enumerate(gkf.split(df, df['label'], groups), 1):
    ...
    train_loader = DataLoader(CroppedDataset(train_df, DATA_DIR, train_tf),batch_size=BATCH_SIZE, shuffle=True, num_workers=0,collate_fn=collate_fn)
    val_loader   = DataLoader(CroppedDataset(val_df, DATA_DIR, eval_tf),batch_size=BATCH_SIZE, shuffle=False,num_workers=0,collate_fn=collate_fn)
...
print(f"\n{NUM_FOLDS}-Fold CV average AUC: {avg_auc:.4f}")
...
print("\nTest Set Performance using best fold model:")
