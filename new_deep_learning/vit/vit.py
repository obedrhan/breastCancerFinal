#!/usr/bin/env python3
"""
train_vit_cropped.py

Full‐image ViT‐base fine‐tuning on CBIS‐DDSM cropped ROIs,
using final_cropped_full.csv and best practices from the literature.
"""

import os
import random
import cv2
import numpy as np
import pandas as pd
from PIL import Image
import pydicom

import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import Dataset, DataLoader
from torchvision import transforms
from sklearn.model_selection import GroupShuffleSplit
from sklearn.metrics import (
    accuracy_score, precision_score, recall_score, f1_score, roc_auc_score
)
from transformers import ViTForImageClassification

# ─── CONFIG ─────────────────────────────────────────────────────
BASE_DIR     = "/Users/ecekocabay/Desktop/2025SPRING/ CNG492/DDSM"
CSV_PATH     = os.path.join(BASE_DIR, "data/final_cropped_full.csv")
CKPT_PATH    = os.path.join(BASE_DIR, "new_deep_learning/vit/best_vit_cropped.pth")
BATCH_SIZE   = 16
LR_HEAD      = 5e-5
LR_FINETUNE  = 5e-6
EPOCHS_HEAD  = 5
EPOCHS_FULL  = 20
PATIENCE     = 4
VAL_FRAC     = 0.15
DEVICE       = torch.device("mps") if torch.backends.mps.is_available() else torch.device("cpu")
SEED         = 42

# ─── REPRO ───────────────────────────────────────────────────────
random.seed(SEED)
np.random.seed(SEED)
torch.manual_seed(SEED)

# ─── DATASET ─────────────────────────────────────────────────────
class CroppedViTDataset(Dataset):
    def __init__(self, df, transform=None):
        self.df = df.reset_index(drop=True)
        self.transform = transform

        # normalize pathology labels
        self.df['path_low'] = (
            self.df['pathology_x']
              .str.lower().str.strip()
              .replace({'benign without callback':'benign'})
        )
        self.df['label'] = self.df['path_low'].map({'benign':0,'malignant':1})
        self.df.dropna(subset=['label'], inplace=True)
        self.df['label'] = self.df['label'].astype(int)

    def __len__(self):
        return len(self.df)

    def __getitem__(self, idx):
        row = self.df.iloc[idx]
        path = os.path.join(BASE_DIR, row['full_path_x'])
        ds = pydicom.dcmread(path, force=True)
        arr = ds.pixel_array.astype(np.float32)

        # normalize to [0,255]
        arr = np.uint8(255 * (arr - arr.min())/(arr.max()-arr.min()+1e-6))
        # CLAHE
        arr = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8)).apply(arr)

        img = Image.fromarray(arr).convert('RGB')
        if self.transform:
            img = self.transform(img)

        return img, row['label']

# ─── SPLIT METADATA ─────────────────────────────────────────────
df = pd.read_csv(CSV_PATH)

# Test = any path containing "Test_"
test_df = df[df['full_path_x'].str.contains('test_', case=False)].reset_index(drop=True)
# Train+val = "Training_"
tv_df   = df[df['full_path_x'].str.contains('training_', case=False)].reset_index(drop=True)

gss = GroupShuffleSplit(n_splits=1, test_size=VAL_FRAC, random_state=SEED)
train_idx, val_idx = next(gss.split(tv_df, groups=tv_df['patient_id']))
train_df = tv_df.iloc[train_idx].reset_index(drop=True)
val_df   = tv_df.iloc[val_idx].reset_index(drop=True)

# ─── TRANSFORMS ─────────────────────────────────────────────────
train_tf = transforms.Compose([
    transforms.RandomResizedCrop(224, scale=(0.9,1.1)),
    transforms.RandomHorizontalFlip(),
    transforms.ToTensor(),
    transforms.Normalize([0.5]*3, [0.5]*3)
])
eval_tf = transforms.Compose([
    transforms.Resize((224,224)),
    transforms.ToTensor(),
    transforms.Normalize([0.5]*3, [0.5]*3)
])

# ─── DATALOADERS ────────────────────────────────────────────────
train_ds = CroppedViTDataset(train_df, transform=train_tf)
val_ds   = CroppedViTDataset(val_df,   transform=eval_tf)
test_ds  = CroppedViTDataset(test_df,  transform=eval_tf)

from torch.utils.data._utils.collate import default_collate
def collate_fn(batch):
    batch = [b for b in batch if b is not None]
    return default_collate(batch)

train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True,
                          num_workers=0, collate_fn=collate_fn)
val_loader   = DataLoader(val_ds,   batch_size=BATCH_SIZE, shuffle=False,
                          num_workers=0, collate_fn=collate_fn)
test_loader  = DataLoader(test_ds,  batch_size=BATCH_SIZE, shuffle=False,
                          num_workers=0, collate_fn=collate_fn)

# ─── MODEL UTILS ────────────────────────────────────────────────
def get_vit():
    model = ViTForImageClassification.from_pretrained(
        "google/vit-base-patch16-224",
        num_labels=2,
        ignore_mismatched_sizes=True
    )
    return model.to(DEVICE)

# ─── SETUP LOSS/OPT ─────────────────────────────────────────────
vit = get_vit()

# class weights
lbls = train_ds.df['label'].values
cnts = np.bincount(lbls)
cw   = torch.tensor([1/cnts[0], 1/cnts[1]], dtype=torch.float32, device=DEVICE)
criterion = nn.CrossEntropyLoss(weight=cw)

optimizer = optim.AdamW(vit.parameters(), lr=LR_HEAD, weight_decay=1e-2)
scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    optimizer, mode='max', factor=0.5, patience=2
)

best_val_auc = 0.0
patience_cnt = 0

# ─── TRAINING LOOP ──────────────────────────────────────────────
# Stage 1: head only
for p in vit.vit.parameters():
    p.requires_grad = False

for epoch in range(1, EPOCHS_HEAD+1):
    vit.train()
    y_true, y_pred = [], []
    for imgs, labs in train_loader:
        imgs, labs = imgs.to(DEVICE), labs.to(DEVICE)
        out = vit(imgs).logits
        loss = criterion(out, labs)
        optimizer.zero_grad(); loss.backward(); optimizer.step()

        preds = out.argmax(1).cpu().numpy()
        y_pred.extend(preds); y_true.extend(labs.cpu().numpy())

    train_acc = accuracy_score(y_true, y_pred)

    # validate
    vit.eval()
    v_true, v_score = [], []
    with torch.no_grad():
        for imgs, labs in val_loader:
            imgs = imgs.to(DEVICE)
            logits = vit(imgs).logits
            probs  = torch.softmax(logits,1)[:,1].cpu().numpy()
            v_score.extend(probs)
            v_true.extend(labs.numpy())

    val_auc = roc_auc_score(v_true, v_score)
    print(f"[Head] Epoch {epoch:02d}  TrainAcc: {train_acc:.4f}  ValAUC: {val_auc:.4f}")

    if val_auc > best_val_auc:
        best_val_auc, patience_cnt = val_auc, 0
        torch.save(vit.state_dict(), CKPT_PATH)
    else:
        patience_cnt += 1
        if patience_cnt >= PATIENCE:
            break

# Stage 2: fine‐tune all
vit.load_state_dict(torch.load(CKPT_PATH, map_location=DEVICE))
for p in vit.parameters():
    p.requires_grad = True

optimizer = optim.AdamW(vit.parameters(), lr=LR_FINETUNE, weight_decay=1e-2)
scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    optimizer, mode='max', factor=0.5, patience=2
)
best_val_auc, patience_cnt = 0.0, 0

for epoch in range(1, EPOCHS_FULL+1):
    vit.train()
    for imgs, labs in train_loader:
        imgs, labs = imgs.to(DEVICE), labs.to(DEVICE)
        out = vit(imgs).logits
        loss = criterion(out, labs)
        optimizer.zero_grad(); loss.backward(); optimizer.step()

    vit.eval()
    v_true, v_score = [], []
    with torch.no_grad():
        for imgs, labs in val_loader:
            imgs = imgs.to(DEVICE)
            logits = vit(imgs).logits
            probs  = torch.softmax(logits,1)[:,1].cpu().numpy()
            v_score.extend(probs)
            v_true.extend(labs.numpy())

    val_auc = roc_auc_score(v_true, v_score)
    scheduler.step(val_auc)
    print(f"[Full] Epoch {epoch:02d}  ValAUC: {val_auc:.4f}")

    if val_auc > best_val_auc:
        best_val_auc, patience_cnt = val_auc, 0
        torch.save(vit.state_dict(), CKPT_PATH)
    else:
        patience_cnt += 1
        if patience_cnt >= PATIENCE:
            break

print(f"\nBest Val AUC: {best_val_auc:.4f}")

# ─── FINAL TEST ────────────────────────────────────────────────────
vit.load_state_dict(torch.load(CKPT_PATH, map_location=DEVICE))
vit.eval()
t_true, t_pred, t_score = [], [], []
with torch.no_grad():
    for imgs, labs in test_loader:
        imgs = imgs.to(DEVICE)
        logits = vit(imgs).logits
        probs  = torch.softmax(logits,1)[:,1].cpu().numpy()
        preds  = logits.argmax(1).cpu().numpy()
        t_score.extend(probs)
        t_true.extend(labs.numpy())
        t_pred.extend(preds)

test_acc  = accuracy_score(t_true, t_pred)
test_prec = precision_score(t_true, t_pred)
test_rec  = recall_score(t_true, t_pred)
test_f1   = f1_score(t_true, t_pred)
test_auc  = roc_auc_score(t_true, t_score)

print("\nTest Performance:")
print(f" AUC:      {test_auc:.4f}")
print(f" Accuracy: {test_acc:.4f}")
print(f" Precision:{test_prec:.4f}")
print(f" Recall:   {test_rec:.4f}")
print(f" F1 Score: {test_f1:.4f}")