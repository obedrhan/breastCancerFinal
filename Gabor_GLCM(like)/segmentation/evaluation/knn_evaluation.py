...
# === Load evaluation_test features ===
df = pd.read_csv(FEATURE_CSV)
X_test = df.drop(columns=["Image Name", "Label"]).values
y_test = df["Label"].values

# === Load trained model and scaler ===
model_path = os.path.join(MODEL_DIR, "knn_gabor_glcm_segmented.pkl")
scaler_path = os.path.join(MODEL_DIR, "scaler_gabor_glcm_segmented.pkl")  # adjust if your scaler is named differently
